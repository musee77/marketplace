        ...
        extra_context.update(
            {
                ...
                "total_orders": Order.objects.filter(is_simulated=False).count(),
                "pending_orders": Order.objects.filter(is_simulated=False, status=Order.Status.PENDING).count(),
                ...
                "recent_orders": Order.objects.filter(is_simulated=False).select_related("client", "specialist", "service").order_by("-created_at")[:5],
